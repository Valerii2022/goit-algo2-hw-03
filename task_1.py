...
def build_graph():
    ...
    edges = [
        ('T1', 'S1', 10), ('T1', 'S2', 15),
        ('T2', 'S2', 10), ('T2', 'S3', 10),
        ('S1', 'M1', 5), ('S1', 'M2', 5), ('S1', 'M3', 5),
        ('S2', 'M4', 10), ('S2', 'M5', 5), ('S2', 'M6', 5),
        ('S3', 'M7', 10), ('S3', 'M8', 10),
        ('M1', 'M14', 5), ('M4', 'M14', 5), ('M7', 'M14', 5)
    ]
...
def analyze_results(flow_dict, G):
    ...
    print(f"Термінал з найбільшим потоком: {max_terminal} ({terminal_flows[max_terminal]} одиниць)")

    # Визначення 3-х найвужчих місць
    min_capacity_edges = sorted([(u, v, d['capacity']) for u, v, d in G.edges(data=True)], key=lambda x: x[2])[:3]
    ...
    store_flows = {v: sum(flow_dict[u][v] for u in flow_dict if v in flow_dict[u]) for v in G.nodes() if v.startswith('M')}
    min_store = min(store_flows, key=store_flows.get, default="Немає потоку")
    print(f"Магазин з найменшим потоком: {min_store} ({store_flows[min_store]} одиниць)")
